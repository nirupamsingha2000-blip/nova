...
// Used for sitemap.xml/canonical links. Update if the site ever moves to a
// custom domain or a different host.
const SITE_URL = 'https://nirupamsingha2000-blip.github.io/nova';
...
function headMeta(page) {
    ...
    ${page.noindex ? '<meta name="robots" content="noindex, nofollow">' : '<link rel="canonical" href="' + SITE_URL + '/' + page.out.replace(/index\.html$/, '') + '">'}
...
// Pages are hosted at a domain root on some platforms (Netlify) and under a
// repo-name subpath on others (GitHub Pages project sites, e.g. /nova/), so
// every internal link/asset path is written as a relative "{{BASE}}..." token
// in partials/content and resolved here per page, based on its folder depth.
function computeBase(outPath) {
...
function renderPage(page) {
    ...
    const base = computeBase(page.out);
